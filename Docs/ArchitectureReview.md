...
| --- | --- | --- |
| `InnoDI` | `6de46b4` revision | 기존 hierarchy/container surface, experimental `@_InnoDIAssistedFactoryPrototype`, `InnoDIDAGValidationPlugin` |
...
- 파일럿은 `_spi(Experimental)` surface와 InnoDI revision `6de46b4afb961c3741b4146023a4c0b262f6d0cc`에 의도적으로 고정합니다. 6.0 정식 API로 간주하지 않습니다.

현재 prototype이 생성하는 nested factory type은 같은 모듈의 다른 source file에서 직접 참조할 때 Xcode batch compilation에서 안정적으로 노출되지 않았습니다. 그래서 생성 타입을 선언 파일의 `PeopleDetailFactoryPilot`이 감싸고 parent coordinator가 이 wrapper를 소유합니다. 이 제약은 6.0 GA 전에 parent-owned factory contract와 외부 모듈 visibility를 정식으로 고정해야 한다는 실사용 근거입니다.
...
generated workspace가 InnoDI revision `6de46b4afb961c3741b4146023a4c0b262f6d0cc`와 나머지 exact release를 resolve하는지도 별도로 확인합니다. 현재 최종 변경 기준으로 Remote 16개와 feature 25개, 총 41개 테스트 및 generic iOS app/embedded watch app build를 포함한 전체 gate가 통과했습니다.

## Recommended Next Sequence

1. InnoDI 6.0 parent-owned assisted factory contract가 확정되면 `PeopleDetailFactoryPilot` wrapper를 정식 surface로 교체
